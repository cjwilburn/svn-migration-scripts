/**
 * Copyright 2015 Courtney Wilburn, Atlassian
 *
...
object GithubPush extends Command {
  val name = "github-push"
  val help = "Push to a repository on Github, optionally creating it."
  ...
  def existing(http: Http, api: Request, name: String, owner: String): Option[String] =
    http(api / "user" / "repositories" ># { json =>
      ...
      case ex: StatusCode =>
        System.err.println("Could not verify if the repository exists on Github: check the username or the password are valid")
  ...
  def push(git: Git, remote: String): Either[String, String] =
    Either.cond((git("git", "push", "--progress", "--all", remote) #&& git("git", "push", "--progress", "--tags", remote)).! == 0,
      "Successfully pushed to Github", "Pushing repository to Github failed.")
  ...
  def apply(cmd: Cmd, options: Array[String], arguments: Array[String]): Boolean = {
    ...
    (for {
      slug <- repoSlug(:/("api.github.com").secure.as_!(username, password) / "1.0", name, owner).right
      remote <- {
        if (options.contains("--ssh"))
          ensureRemote(git, "github-ssh", "git@github.com:%s/%s".format(owner, slug))
        else
          ensureRemote(git, "github", "https://%s@github.com/%s/%s".format(e(username), owner, slug))
      }.right
